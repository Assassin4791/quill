...
abstract class JAsyncContextConfig[C <: ConcreteConnection](
  ...
  lazy val connectionPoolConfiguration = new ConnectionPoolConfiguration(
    ...
    getLong("queryTimeout").orElse(Option(urlConfiguration.getQueryTimeout).map(_.toMillis)).map(new JavaLong(_)).orNull,
    ...
    getLong("maxConnectionTtl").map(new java.lang.Long(_)).orElse(Option(default.getMaxConnectionTtl)).orNull
  )
